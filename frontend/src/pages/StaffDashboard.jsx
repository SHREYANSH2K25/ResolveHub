  ...
  const ComplaintModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white dark:bg-municipal-800 rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
          <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-municipal-700">
            ...
            <button
              ...
              className="p-2 hover:bg-gray-100 dark:hover:bg-municipal-700 rounded-full transition-colors"
            >
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)] grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">{selectedComplaint.title}</p>
              </div>
              <div>
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Submitted By</h4>
                <p className="text-gray-700 dark:text-gray-300">
                  {selectedComplaint.submittedBy.name} ({selectedComplaint.submittedBy.email})
                </p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Submitted Date</h4>
                <p className="text-gray-700 dark:text-gray-300">{formatDate(selectedComplaint.createdAt)}</p>
              </div>
              {selectedComplaint.resolutionDate && (
                <div>
                  <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Resolution Date</h4>
                  <p className="text-gray-700 dark:text-gray-300">{formatDate(selectedComplaint.resolutionDate)}</p>
                </div>
              )}
            </div>
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  {selectedComplaint.status === 'OPEN' && (
                    <button
                      onClick={() => updateStatus(selectedComplaint._id, 'IN PROGRESS')}
                      disabled={updating === selectedComplaint._id}
                      className="btn-primary flex items-center space-x-2"
                    >
                      {updating === selectedComplaint._id ? <LoadingSpinner size="sm" /> : <Clock className="w-4 h-4" />}
                  ...
                  {(selectedComplaint.status === 'OPEN' || selectedComplaint.status === 'IN PROGRESS') && (
                    ...
                    >
                      {updating === selectedComplaint._id ? <LoadingSpinner size="sm" /> : <CheckCircle className="w-4 h-4" />}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-300">Loading staff dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard title="Total Complaints" value={stats.total} icon={<Users className="w-6 h-6" />} color="blue" />
          <StatCard title="Open Issues" value={stats.open} icon={<AlertTriangle className="w-6 h-6" />} color="red" />
          <StatCard title="In Progress" value={stats.inProgress} icon={<Clock className="w-6 h-6" />} color="yellow" />
          <StatCard title="Resolved" value={stats.resolved} icon={<CheckCircle className="w-6 h-6" />} color="green" />
        </div>
          ...
          {filteredComplaints.length === 0 ? (
            <Card className="text-center py-12">
              ...
              <p className="text-gray-600 dark:text-gray-300">
                {complaints.length === 0 ? 'Great job! All complaints are up to date.' : 'Try adjusting your search or filter criteria.'}
              </p>
            </Card>
          ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      {c.category && <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-xs rounded-full">{c.category}</span>}
                    </div>
                    <p className="text-gray-600 dark:text-gray-300 mb-3 line-clamp-2">{c.description}</p>
                    <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                      <span className="flex items-center space-x-1"><Users className="w-4 h-4" /><span>{c.submittedBy.name}</span></span>
                      <span className="flex items-center space-x-1"><Calendar className="w-4 h-4" /><span>{formatDate(c.createdAt)}</span></span>
                      {c.mediaUrls?.length > 0 && <span className="flex items-center space-x-1"><ImageIcon className="w-4 h-4" /><span>{c.mediaUrls.length} file{c.mediaUrls.length !== 1 ? 's' : ''}</span></span>}
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 ml-4">
                    <button onClick={() => setSelectedComplaint(c)} className="btn-secondary flex items-center space-x-2"><Eye className="w-4 h-4" /><span>View</span></button>
                    {c.status === 'OPEN' && (
                      <button
                        ...
                        className="btn-primary flex items-center space-x-2"
                      >
                        {updating === c._id ? <LoadingSpinner size="sm" /> : <Clock className="w-4 h-4" />}
                        <span>Start</span>
                      </button>
                    )}
                    {(c.status === 'OPEN' || c.status === 'IN PROGRESS') && (
                      <button
                        ...
                        className="btn-primary bg-green-600 hover:bg-green-700 flex items-center space-x-2"
                      >
                        {updating === c._id ? <LoadingSpinner size="sm" /> : <CheckCircle className="w-4 h-4" />}
                        <span>Resolve</span>
                      </button>
                    )}
                  </div>
